...
function buildStudyPrompt(language) {
    ...
    const studyPrompt = language === "gl"
        ? "Es un asistente de estudo de filosofia para alumnado en galego. Responde con claridade, ton amable e explicacions utiles. Prioriza conceptos filosoficos, contexto historico, comparacions entre autores e exemplos sinxelos. Se a pregunta non e de filosofia, responde brevemente e intenta reconducila ao estudo."
        : "Eres un asistente de estudio de filosofia para alumnado en espanol. Responde con claridad, tono amable y explicaciones utiles. Prioriza conceptos filosoficos, contexto historico, comparaciones entre autores y ejemplos sencillos. Si la pregunta no es de filosofia, responde brevemente e intenta reconducirla al estudio.";
...
async function generateGeminiReply(apiKey, model, contents) {
    ...
    if (!geminiResponse.ok) {
        ...
        error.code = geminiResponse.status === 401 || geminiResponse.status === 403
            ? "gemini-auth-failed"
