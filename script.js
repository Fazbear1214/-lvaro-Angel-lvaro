const translations = {
    es: {
        ...
        statPracticeValue: "27",
        statPracticeLabel: "preguntas interactivas",
        ...
        philosophersText: "Una selección ampliada de autores con fichas completas para estudiar contexto, ideas e influencia.",
        ...
        activitiesEyebrow: "Práctica interactiva",
        activitiesTitle: "Pon a prueba tu conocimiento filosófico",
        activitiesText: "Elige los filósofos que quieres repasar y responde preguntas basadas en las páginas de cada autor.",
        activitiesButton: "Entrar al test",
        quizPreviewTag: "Modo personalizado",
        quizPreviewTitle: "Selecciona autores, responde y revisa tus aciertos",
        quizPreviewAuthors: "filósofos",
        quizPreviewQuestions: "preguntas",
        quizPreviewText: "Las preguntas salen de la información desarrollada en las fichas tipo artículo.",
        quizTag: "Quiz rápido",
        ...
        chatStatus: "Chat preparado para responder con Gemini.",
        chatInputLabel: "Escribe tu pregunta",
        chatPlaceholder: "Haz una pregunta sobre filosofía...",
        chatButton: "Enviar",
        chatWelcome: "Hola. Estoy listo para ayudarte a estudiar filosofía con Gemini.",
        chatThinking: "Pensando...",
        chatConfigError: "Falta la API key de Gemini en el servidor local. Revisa el archivo .env.",
        chatAuthError: "Gemini ha rechazado la API key. Crea una clave nueva y cámbiala en .env.",
        chatOverloadedError: "Gemini está saturado ahora mismo. Prueba otra vez en unos segundos.",
        chatQuotaError: "Has alcanzado el límite o la cuota de Gemini para esta clave. Espera un poco o revisa los límites de la cuenta.",
        chatGenericError: "Gemini no ha aceptado la petición. Prueba de nuevo en unos segundos.",
        chatUnavailable: "El chat no está disponible aquí. En GitHub Pages no se ejecuta el servidor (/api/chat). Ejecuta server.js en local o publica un backend y configura la URL con <meta name=\"chat-api\" content=\"https://.../api/chat\">.",
        chatNetworkError: "No he podido contactar con el servidor local. Abre la web desde http://localhost:3000.",
    ...
    gl: {
        ...
        philosophersText: "Unha selección ampliada de autores con fichas completas para estudar contexto, ideas e influencia.",
        ...
        activitiesEyebrow: "Práctica interactiva",
        activitiesTitle: "Pon a proba o teu coñecemento filosófico",
        activitiesText: "Escolle os filósofos que queres repasar e responde preguntas baseadas nas páxinas de cada autor.",
        activitiesButton: "Entrar ao test",
        quizPreviewTag: "Modo personalizado",
        quizPreviewTitle: "Selecciona autores, responde e revisa os teus acertos",
        quizPreviewAuthors: "filósofos",
        quizPreviewQuestions: "preguntas",
        quizPreviewText: "As preguntas saen da información desenvolvida nas fichas tipo artigo.",
        quizTag: "Quiz rápido",
        ...
        chatButton: "Enviar",
        chatWelcome: "Ola. Estou listo para axudarche a estudar filosofía con Gemini.",
        chatThinking: "Pensando...",
        chatConfigError: "Falta a API key de Gemini no servidor local. Revisa o arquivo .env.",
        chatAuthError: "Gemini rexeitou a API key. Crea unha clave nova e cámbiaa en .env.",
        chatOverloadedError: "Gemini esta saturado agora mesmo. Proba outra vez nuns segundos.",
        chatQuotaError: "Acadaches o limite ou a cota de Gemini para esta clave. Agarda un pouco ou revisa os limites da conta.",
        chatGenericError: "Gemini non aceptou a petición. Proba de novo nuns segundos.",
        chatUnavailable: "O chat non está dispoñible aquí. En GitHub Pages non se executa o servidor (/api/chat). Executa server.js en local ou publica un backend e configura a URL con <meta name=\"chat-api\" content=\"https://.../api/chat\">.",
        chatNetworkError: "Non puiden contactar co servidor local. Abre a web desde http://localhost:3000.",
...
const philosophyData = window.PHILOSOPHY_DATA || { philosophers: [] };
...
const philosopherSearch = document.getElementById("philosopher-search");
const filterButtons = document.querySelectorAll("[data-era]");
...
function getPhilosopherCards() {
    if (philosophyData.philosophers.length) {
        return philosophyData.philosophers.map((item) => ({
            id: item.id,
            name: item.name,
    ...
    return translations[state.language].philosophers.map((item) => ({
        ...item,
        id: normalizeText(item.name).replace(/\s+/g, "-")
    }));
}

function getFilteredPhilosophers() {
    const search = normalizeText(state.philosopherSearch);

    return getPhilosopherCards().filter((item) => {
        const matchesEra = state.philosopherEra === "all" || item.era === state.philosopherEra;
...
function renderPhilosophers() {
    const cards = getFilteredPhilosophers();

    philosopherGrid.innerHTML = cards.length
        ? cards
        .map((item) => `
            <a class="philosopher-card" href="filosofo.html?id=${item.id}" aria-label="Leer más sobre ${item.name}">
                ...
                <div class="philosopher-card-body">
                    ...
                    <p>${item.text}</p>
                    <div class="tag-row">
                        ${(item.themes || []).slice(0, 3).map((theme) => `<span>${theme}</span>`).join("")}
                    </div>
                    <span class="card-link">Leer perfil</span>
                </div>
            </a>
        `)
        .join("")
        : `<p class="empty-state">No hay filósofos que coincidan con ese filtro.</p>`;
}

function updateQuizPreviewMetrics() {
    const authorCount = document.querySelector("[data-quiz-author-count]");
    const questionCount = document.querySelector("[data-quiz-question-count]");

    if (!authorCount || !questionCount || !philosophyData.philosophers.length) {
        return;
    }

    authorCount.textContent = philosophyData.philosophers.length;
    questionCount.textContent = philosophyData.philosophers
        .reduce((total, philosopher) => total + philosopher.questions.length, 0);
}
...
async function sendChatMessage(event) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (data.error === "gemini-auth-failed" || response.status === 401 || response.status === 403) {
                throw new Error("gemini-auth-failed");
            }

            if (data.error === "gemini-overloaded") {
                throw new Error("gemini-overloaded");
            }

            if (data.error === "gemini-quota-exceeded" || response.status === 429) {
        ...
        thinkingNode.textContent = error.message === "missing-api-key"
            ? translations[state.language].chatConfigError
            : error.message === "chat-unavailable"
                ? translations[state.language].chatUnavailable
            : error.message === "gemini-auth-failed"
                ? translations[state.language].chatAuthError
                : error.message === "gemini-overloaded"
                    ? translations[state.language].chatOverloadedError
                    : error.message === "gemini-quota-exceeded"
                    ? translations[state.language].chatQuotaError
                    : error.message === "gemini-request-failed"
                        ? translations[state.language].chatGenericError
...
philosopherSearch?.addEventListener("input", (event) => {
    state.philosopherSearch = event.target.value;
    renderPhilosophers();
});

filterButtons.forEach((button) => {
    button.addEventListener("click", () => {
        state.philosopherEra = button.dataset.era;
...
const phraseCheck = document.getElementById("phrase-check");
if (phraseCheck) {
    phraseCheck.addEventListener("click", handlePhraseCheck);
}
